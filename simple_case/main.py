...
class ClassConditionedUnet(nn.Module):
  def __init__(self, num_classes=10, class_emb_size=4):
    ...
    self.model = UNet2DModel(
        sample_size=28,           # keep original 28x28 image size
        in_channels=1 + class_emb_size, # Additional input channels for class cond.
        out_channels=1,           # the number of output channels
        layers_per_block=3,       # increased from 2 to 3 for more depth
        block_out_channels=(32, 64, 128),  # 3 levels instead of 4, with increased channels
        down_block_types=(
...
if args.sample:
    ...
    try:
        ...
        fig.text(0.85, 0.98, r'Real\\Sample', ha='center', fontsize=14)
        fig.text(0.95, 0.98, r'Class\\Name', ha='center', fontsize=14)
