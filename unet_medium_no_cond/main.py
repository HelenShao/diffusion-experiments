# unet_medium: Medium UNet with 3 levels, 3 layers per block
...
class ClassConditionedUnet(nn.Module):
  def __init__(self, num_classes=10, class_emb_size=4):
    ...
    self.model = UNet2DModel(
        sample_size=28,           # original 28x28 size
        in_channels=1 + class_emb_size, # Additional input channels for class cond.
        out_channels=1,           # the number of output channels
        layers_per_block=3,       # 3 layers per UNet block
        block_out_channels=(32, 64, 128),  # 3 levels
        down_block_types=(
            "DownBlock2D",
            "AttnDownBlock2D",
            "AttnDownBlock2D",
...
if args.sample:
    ...
    # Zero out the labels to remove conditioning
    y = y * 0
